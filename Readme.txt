...
Прокси сервер, по умолчанию, запускается по ip адресу: "127.0.0.1" с номером порта "8889". Номер порта можно изменить в в файле с настройками:
"Proxy_Router/proxy/setting.conf"
...
Ниже приведена таблица соответствия прокси сервера, которая находится в директории "Proxy_Router/proxy/table.xml". 
...
id которых не присутствует в таблице. Адрес этого сервера указан в конфигурационном файлt: "Proxy_Router/server/config.ini"
Есть возможность установить время задержки ответа на запрос. Это можно сделать, так же, в "Proxy_Router/server/config.ini"
...
При отсылке не валидных запросов и получении соответствующей ошибки - устройство прекращает свою работу, соглассно рекомендациям. Его следует перезапустить 
с корректно формируемой структурой запроса. 
...
Его можно изменить в конфигурационном файле: "Proxy_Router/device/config.ini".

Так же, в "Proxy_Router/device/config.ini" можно установить таймаут для запроса.

В процессе работы системы будут формироваться и заполняться log-файлы. 

- Для устройств это будут: "Proxy_Router/device/(id устройства)_device.log".
- Для прокси это будет: "Proxy_Router/proxy/info_proxy.log".
- Для серверов это будут: "Proxy_Router/server/(номер порта)_server.log"
